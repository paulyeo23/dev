...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        new_node = (id, name)
        original_len = len(self.unique_nodes)
        self.unique_nodes.add(new_node)
        new_len = len(self.unique_nodes)
...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id: str, limit: int = None, exclude_ids: list[int] = None) -> list:
        ...
        conn.request(
            "GET", f"/3/movie/{movie_id}/credits", headers=self.headers)

        response = conn.getresponse()

        data = response.read()['cast']

        if exclude_ids:
            filtered_data = filter(
                lambda obj: obj['id'] not in exclude_ids, data)
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        conn.request(
            "GET", f"/3/person/{person_id}/movie_credits", headers=self.headers)
        ...
        minimum_date = start_date or datetime.strptime(
            "1990-01-01", "%Y-%m-%d").date()
        maximum_date = end_date or datetime.strptime(
            "3000-01-01", "%Y-%m-%d").date()

        filtered_data = filter(lambda obj: datetime.strptime(
            obj['release_date'], "%Y-%m-%d").date() >= minimum_date and datetime.strptime(
            obj['release_date'], "%Y-%m-%d").date() <= maximum_date, data)
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(
        api_key='eyJhbGciOiJIUzI1NiJ9.eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJlOTA5NGE1M2QzNGNiNGIxNTQ4YzJiYWY3ZDgzYmM0YiIsIm5iZiI6MTc1NTgyNTgwNy4xMjQsInN1YiI6IjY4YTdjNjhmNmEzNjY5MGMwMzk2NTQ1MyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.7Zipp13z0MXEZi0K-viGAUDQ19fEbQ-z5V6ltyh28I4.7Zipp13z0MXEZi0K-viGAUDQ19fEbQ-z5V6ltyh28I4')

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK
    movies = tmdb_api_utils.get_movie_credits_for_person(
        '2975', '1999-01-01', '1999-12-31')

    for movie in movies:
        movie_id = movie['id']
        movie_cast = tmdb_api_utils.get_movie_cast(
            movie_id, 5, graph.actor_ids)
